...
这段代码定义了一个 Rust 结构体 `Button`，用于在屏幕上绘制一个按钮。为了简单起见，我们没有在示例中演示如何把按钮渲染出来。除了结构体定义，代码还包括调用 `live_design` 宏。该宏用于定义一个 Live 代码块。Live 代码块看起来很像 JSON 文档，但还是有一些重要的区别。
...
其次要注意的是以 `fn` 开头的代码块。它们表示函数表达式的开始。函数表达式的作用类似于 Lisp 中的 quote。函数表达式并没有被完全解析，而是以词条列表（a list of tokens）的形式被嵌入，然后可以根据被应用的不同类型再对其做不同的解释。
...
Live 代码块是以 UTF-8 编码的 Unicode 码点序列。Live 代码块的词法结构将这些码点组合成具有特定含义的序列，也就是词条（tokens）。词条可以是注释、标识符、关键字、文字、标点符号或分隔符。接下来的部分，将介绍这些词条的更多细节。由于 Live 代码的词法结构意在尽可能与 Rust 的词法结构相匹配，所以后续会指明与 Rust 的差异之处。
...
字面量是一个单独的词条，用于直接表示一个值。下面列举了 Live 代码支持的各种字面量。
...
Live 代码块的词法结构是一个词条序列。Live 代码块的语法结构将这些词条进一步组织成一个解析树（parse tree）。解析树的顶层总是由一系列项组成。项又可以包含表达式。本节的其余部分将会详细介绍项和表达式。
...
此外，每个节点都有一个原点（origin），定义了节点在词条流（token stream）中的定义位置，还有一组标志（flags），标志有多种用途，例如表示节点所代表的属性类别（the kind of property）、包括 *字段*、*实例*和*模版*，或者指示名称是否包含前缀。请注意，节点中并不包含前缀，但可以使用原点从词条流中获取。
...
| `close` | 表示数组或对象结束的特殊值 |
| `fn(tokens)` | function 类型的值<br /> 该类型的值表示指向词条列表的指针，根据 Rust 结构体的不同，对其的解释（interpreted）也不同。 |
...
函数表达式的转译方案非常简单，只需将组成表达式的词条列表转换为指向该列表指针的节点：

```math
T[[fn\ (token\_list)\ (->\ identifier)^?\ \{\ token\_list\ \}]] = fn(tokens)
```

其中 *tokens* 是指向组成表达式的词条列表的指针。
